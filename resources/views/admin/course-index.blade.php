<x-layouts.admin title="Kelola Kelas">
    ...
    <div class="flex justify-end mb-6">
        <a href="/admin/kelas/create" class="bg-blue-700 text-white text-sm font-semibold px-5 py-2.5 rounded-lg hover:bg-blue-800">+ Tambah Kelas</a>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-gray-500 text-xs uppercase">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left">Pelajaran</th>
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse($courses as $course)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-gray-500">{{ $course->lessons_count }} pelajaran</td>
                        <td class="px-6 py-4">Rp {{ number_format($course->price, 0, ',', '.') }}</td>
                        <td class="px-6 py-4">
                            <span @class([
                                'text-xs font-semibold px-2 py-1 rounded-full',
                                'bg-green-100 text-green-700' => $course->is_published,
                                'bg-gray-100 text-gray-500'   => !$course->is_published,
                            ])>{{ $course->is_published ? 'Publik' : 'Draft' }}</span>
                        </td>
                ...
                @empty
                    <tr><td colspan="5" class="px-6 py-10 text-center text-gray-400">Belum ada kelas.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
